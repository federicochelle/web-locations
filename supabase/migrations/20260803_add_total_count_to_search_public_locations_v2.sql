...
as $$
  with base_locations as (
    ...
    where l.published = true
      ...
      and (
        nullif(trim(p_department_slug), '') is null
  ...
  structured_filters as (
    select
      (
        nullif(trim(p_category_slug), '') is not null
        or nullif(trim(p_department_slug), '') is not null
        or exists (
          select 1
          from unnest(coalesce(p_feature_slugs, '{}'::text[])) as selected_feature(selected_slug)
          where selected_slug <> ''
  ...
  fallback_results as (
    select
      ...
      ss.selected_tag_count
    from search_source ss
    cross join structured_filters sf
    where sf.has_structured_filters
      and not exists (
        select 1
        from strict_results
      )
  ...
  select
    final_with_total.id,
    final_with_total.slug,
    final_with_total.location_code,
    final_with_total.category_name,
    final_with_total.department_name,
    final_with_total.zone_name,
    final_with_total.cover_image_url,
    final_with_total.cover_image_alt,
    final_with_total.features,
    final_with_total.matched_feature_count,
    final_with_total.selected_feature_count,
    final_with_total.matched_tag_count,
    final_with_total.selected_tag_count,
    final_with_total.total_count
  from final_with_total
  order by final_with_total.location_code asc nulls last, final_with_total.id asc
