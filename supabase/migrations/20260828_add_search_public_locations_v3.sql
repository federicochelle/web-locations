...
create or replace function public.normalize_location_code(input text)
returns text
language sql
immutable
parallel safe
  ...
  select
    case
      when input is null then null
        ...
        regexp_replace(
          coalesce(public.normalize_search_text(input), ''),
          '[^a-z0-9]+',
          '',
          'g'
        ),
...
drop function if exists public.search_public_locations_v3(
  text,
  text[],
  text,
  integer
      ...
      coalesce((
        select sum(
          case
            ...
            when ns.combined_search_text like '%' || optional_term || '%' then 1
            else 0
          end
        )::integer
