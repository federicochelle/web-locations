      ...
      case
        ...
        when signal_source.is_code_like_query and signal_source.code_prefix_match then 1800::numeric
          ...
          (
            ...
            + (signal_source.matched_feature_count::numeric * 420)
            + (signal_source.free_text_term_match_count::numeric * 210)
            + round(signal_source.free_text_rank::numeric * 280, 3)
            + round(signal_source.core_semantic_rank::numeric * 120, 3)
            + round(signal_source.core_feature_rank::numeric * 130, 3)
            + round(signal_source.core_category_rank::numeric * 90, 3)
            + (signal_source.matched_tag_count::numeric * 28)
            + (signal_source.core_term_match_count::numeric * 44)
            + round(signal_source.semantic_trigram::numeric * 75, 3)
          )
      end as search_score,
      case
  ...
  scored as (
    select
      ...
      (
        ...
        + (signal_source.matched_feature_count::numeric * 250)
        + (signal_source.free_text_term_match_count::numeric * 240)
        + round(signal_source.free_text_rank::numeric * 320, 3)
        + round(signal_source.core_semantic_rank::numeric * 180, 3)
        + round(signal_source.core_feature_rank::numeric * 170, 3)
        ...
        + round(signal_source.semantic_trigram::numeric * 90, 3)
      ) as search_score,
      case
        when signal_source.matched_feature_count > 0
